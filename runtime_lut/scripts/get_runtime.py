...
def load_pb(model_path):
    ...
    model_init_path = (model_path + "/model_init.pb") \
        if os.path.exists(model_path + "/model_init.pb") else None

    model, _ = model_utils.load_model_pb(model_path, model_init_path,
        is_run_init=True)
...
def main():
    ...
    for model_path in model_paths:
        ...
        op_time, net_time, all_found = api.get_net_runtime(model, blobs,
            args.input_dims, args.input_dtype, args.device, verbose=True)
