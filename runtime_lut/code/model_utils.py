...
def get_blobs_as_dict(blob_names):
    ret = {
        x: _get_blob(x) for x in blob_names
    }
    return ret
...
def get_blobs_shapes(blobs):
    blob_dims = {
        x: _shape_to_list(_get_blob_shape(blobs[x])) for x in blobs
...
def get_blobs_dtypes(blobs):
    blob_dtypes = {
        x: _get_blob_dtype(blobs[x]) for x in blobs
        if blobs[x] is not None and not isinstance(blobs[x], (str, bytes))
    }
    return blob_dtypes
...
def rand_blob(blob, scale=0.01, zero_point=0):
    ...
    values = np.array(np.random.randint(0, 255, size=shape),
        dtype=data_type)
    scale = scale
...
def infer_model_shape_by_ops(net, param_init_net=None,
                             extra_inputs=None,
                             is_run_gpu=False,
                             skip_op_idx=None,
                             get_dtype=False):
