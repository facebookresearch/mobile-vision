...
def get_ops_from_net(model, blobs, input_dims):
    ...
    blobs = {x: np.zeros(blobs_dims[x], dtype=blobs_dtypes[x])
        for x in blobs_dims}
    ...
    for op in model_ops:
        ...
        param_shape = [np.array(blobs[str(param_blob)]).shape
            for param_blob in op_inputs]
        param_dtypes = [encode_dtype(str(blobs[str(param_blob)].dtype))
            for param_blob in op_inputs]
...
class RunTimeAPI(object):
    ...
    def get_op_runtime(self, op_query, mode='ave', verbose=True):
        ...
        if verbose:
            print('op type {} with input {}'.format(
                op_query.get_val('op_type'), op_query.get_val('input_shapes')))
    ...
    def get_net_runtime(self, model, blobs, input_dims, input_dtype=core.DataType.FLOAT,
            device="SM-G950U-7.0-24", mode='ave', verbose=True):
        """
        ...
        ops, input_shapes, input_dtypes = get_ops_from_net(model, blobs,
            input_dims)
        ...
        while True:
            ...
            for i, op in enumerate(ops):
                op_query = LUTSchema()
                load_caffe2_op(op_query, op,
                    input_shapes=input_shapes[i],
                    input_dtypes=input_dtypes[i],
                    device=devices[i],
                    model_name=str(model))

                latency = self.get_op_runtime(op_query, mode=mode,
                    verbose=verbose)
