...
class TestCorrelation(object):
    def _mult_with_shape(self, shape):
        ...
        * channels = 1
        """
        assert shape[1] == 1
        model = self.get_correlation_block()(k=0, d=0, s1=1, s2=1)
    ...
    def test_displacement_scalar(self):
        ...
        Given displacement d = 1, we have D = 2d + 1 and D ** 2 = 9 output channels
        ...
        There are D ** 2 - 1 zero values.
        """
        d = random.randint(1, 10)
        D = 2 * d + 1
        model = self.get_correlation_block()(k=0, d=d, s1=1, s2=1)

        x1 = torch.randn(1, 1, 1, 1)
        x2 = torch.randn(1, 1, 1, 1)
        output = model(x1, x2)
        gt = F.pad(x1 * x2, (0, 0, 0, 0, (D ** 2 - 1) // 2, (D ** 2 - 1) // 2))
        self.assertEqual(output.shape, torch.Size([1, D ** 2, 1, 1]))
        torch.testing.assert_allclose(output, gt)
    ...
    def test_displacement_scalar_batch_channels(self):
        ...
        output = model(x1, x2)
        gt = (x1 * x2).sum(dim=1).unsqueeze(1) / c
        gt = F.pad(gt, (0, 0, 0, 0, (D ** 2 - 1) // 2, (D ** 2 - 1) // 2))
        self.assertEqual(output.shape, torch.Size([n, D ** 2, 1, 1]))
        torch.testing.assert_allclose(output, gt)

    def test_displacement_1d(self):
        """Displacement > 0 with 1d inputs

        Given displacement d = 1, and 1d inputs we have:
    ...
    def test_displacement_2d(self):
        ...
        Given displacement d = 1, and 2d inputs we have:
    ...
    def test_stride2_1dx(self):
        ...
        d = 2
        D = 2 * d // s2 + 1
        model = self.get_correlation_block()(k=0, d=d, s1=1, s2=s2)
    ...
    def test_stride2_1dy(self):
        ...
        Given displacement d = 2, s2 = 2, h=3
    ...
    def test_stride1_1dx(self):
        ...
        s1 = 2
        model = self.get_correlation_block()(k=0, d=0, s1=s1, s2=1)
        x1 = torch.randn(1, 1, 1, 3)
        x2 = torch.randn(1, 1, 1, 3)
        output = model(x1, x2)
    ...
    def get_correlation_block(self):
        raise NotImplementedError


class TestNaiveCorrelationBlock(unittest.TestCase, TestCorrelation):
    def get_correlation_block(self):
        return NaiveCorrelationBlock


class TestMatMulCorrelationBlock(unittest.TestCase, TestCorrelation):
    def get_correlation_block(self):
        return MatMulCorrelationBlock


class TestUnfoldCorrelationBlock(unittest.TestCase, TestCorrelation):
    def get_correlation_block(self):
        return UnfoldCorrelationBlock


class TestTorchscriptNaiveCorrelation(unittest.TestCase, TestCorrelation):
    def get_correlation_block(self):
        return TorchscriptNaiveCorrelation


class TestTorchscriptUnfoldCorrelation(unittest.TestCase, TestCorrelation):
    def get_correlation_block(self):
        return TorchscriptUnfoldCorrelation
